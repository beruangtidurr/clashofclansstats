// src/components/CwlHistory.js
'use client';

import { useState, useMemo } from 'react';
import Image from 'next/image';

function formatSeason(seasonStr) {
  if (!seasonStr) return '';
  const parts = seasonStr.split('-');
  if (parts.length >= 2) {
    const year = parseInt(parts[0], 10);
    const month = parseInt(parts[1], 10) - 1;
    const date = new Date(Date.UTC(year, month, 1));
    return date.toLocaleDateString('en-US', {
      month: 'long',
      year: 'numeric',
      timeZone: 'UTC',
    });
  }
  return seasonStr;
}

function formatDuration(seconds) {
  if (typeof seconds !== 'number' || seconds < 0) return '';
  const m = Math.floor(seconds / 60);
  const s = seconds % 60;
  return `${m}m ${s.toString().padStart(2, '0')}s`;
}

function ShieldIcon() {
  return (
    <svg className="w-3.5 h-3.5 shrink-0 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
    </svg>
  );
}

export default function CwlHistory({ cwl, loading, error }) {
  // Initialize with the most recent season expanded
  const [expandedSeasons, setExpandedSeasons] = useState({ 0: true });

  const toggleSeason = (index) => {
    setExpandedSeasons((prev) => ({
      ...prev,
      [index]: !prev[index],
    }));
  };

  // Career stats across all seasons
  const careerStats = useMemo(() => {
    if (!Array.isArray(cwl) || cwl.length === 0) return null;

    let totalAttacks = 0;
    let totalMissed = 0;
    let totalStars = 0;
    let threeStars = 0;
    let totalDestruction = 0;

    cwl.forEach((season) => {
      totalMissed += season.missedAttacks ?? 0;
      (season.attacks || []).forEach((att) => {
        totalAttacks += 1;
        totalStars += att.stars ?? 0;
        if (att.stars === 3) threeStars += 1;
        totalDestruction += att.destructionPercentage ?? 0;
      });
    });

    return {
      seasonsCount: cwl.length,
      totalAttacks,
      totalMissed,
      totalStars,
      threeStarRate: totalAttacks > 0 ? Math.round((threeStars / totalAttacks) * 100) : 0,
      avgDestruction: totalAttacks > 0 ? Math.round(totalDestruction / totalAttacks) : 0,
    };
  }, [cwl]);

  if (loading) {
    return (
      <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 p-6 bg-white dark:bg-neutral-900/40 animate-pulse space-y-4">
        <div className="h-5 bg-neutral-100 dark:bg-neutral-800 rounded w-1/4" />
        <div className="grid grid-cols-4 gap-2">
          <div className="h-14 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
          <div className="h-14 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
          <div className="h-14 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
          <div className="h-14 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
        </div>
        <div className="space-y-3 pt-2">
          <div className="h-20 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
          <div className="h-20 bg-neutral-100 dark:bg-neutral-800 rounded-xl" />
        </div>
      </div>
    );
  }

  if (error && (!cwl || cwl.length === 0)) {
    return (
      <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 p-8 bg-white dark:bg-neutral-900/40 text-center">
        <p className="text-sm text-neutral-500 dark:text-neutral-400">
          Unable to load CWL history: {error}
        </p>
      </div>
    );
  }

  if (!cwl || cwl.length === 0) {
    return (
      <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 p-8 bg-white dark:bg-neutral-900/40 text-center">
        <p className="text-sm text-neutral-500 dark:text-neutral-400">
          No Clan War Leagues history recorded for this player.
        </p>
      </div>
    );
  }

  return (
    <div className="rounded-2xl border border-neutral-200/70 dark:border-neutral-800/70 p-5 sm:p-6 bg-white dark:bg-neutral-900/40">
      {/* Header */}
      <div className="flex items-center justify-between gap-2 mb-5">
        <div className="flex items-center gap-2">
          <h3 className="text-base font-semibold text-neutral-900 dark:text-neutral-100">
            Clan War Leagues
          </h3>
          <span className="text-xs px-2 py-0.5 rounded-full font-medium bg-neutral-100 dark:bg-neutral-800 text-neutral-500">
            {cwl.length} {cwl.length === 1 ? 'Season' : 'Seasons'}
          </span>
        </div>
      </div>

      {/* Overall Career Stats Bar */}
      {careerStats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3.5 rounded-xl bg-neutral-50/70 dark:bg-neutral-800/40 border border-neutral-200/50 dark:border-neutral-800/60 mb-5 text-center">
          <div>
            <div className="text-[10px] uppercase tracking-wider font-medium text-neutral-400">
              Total Stars
            </div>
            <div className="text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
              {careerStats.totalStars} ★
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider font-medium text-neutral-400">
              3-Star Rate
            </div>
            <div className="text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
              {careerStats.threeStarRate}%
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider font-medium text-neutral-400">
              Avg Destruction
            </div>
            <div className="text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
              {careerStats.avgDestruction}%
            </div>
          </div>
          <div>
            <div className="text-[10px] uppercase tracking-wider font-medium text-neutral-400">
              Attacks
            </div>
            <div className="text-base font-bold text-neutral-900 dark:text-neutral-100 mt-0.5">
              {careerStats.totalAttacks}
              {careerStats.totalMissed > 0 && (
                <span className="text-rose-500 text-xs font-normal ml-1">
                  ({careerStats.totalMissed} missed)
                </span>
              )}
            </div>
          </div>
        </div>
      )}

      {/* Seasons List */}
      <div className="space-y-3">
        {cwl.map((season, index) => {
          const isExpanded = !!expandedSeasons[index];
          const attacks = [...(season.attacks || [])].sort((a, b) => (a.round || 0) - (b.round || 0));

          const seasonStars = attacks.reduce((sum, a) => sum + (a.stars ?? 0), 0);
          const maxStars = attacks.length * 3;
          const seasonAvgDestruction = attacks.length > 0
            ? Math.round(attacks.reduce((sum, a) => sum + (a.destructionPercentage ?? 0), 0) / attacks.length)
            : 0;

          return (
            <div
              key={`${season.season}-${index}`}
              className="rounded-xl border border-neutral-200/60 dark:border-neutral-800/70 bg-neutral-50/20 dark:bg-neutral-900/20 overflow-hidden"
            >
              {/* Season Top Header Clickable */}
              <div
                onClick={() => toggleSeason(index)}
                className="p-3.5 hover:bg-neutral-50/80 dark:hover:bg-neutral-800/30 transition-colors cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-3"
              >
                <div className="flex items-center gap-3">
                  {season.clan?.badgeUrls?.small && (
                    <div className="relative w-7 h-7 shrink-0">
                      <Image
                        src={season.clan.badgeUrls.small}
                        alt={season.clan.name || 'Clan Badge'}
                        fill
                        sizes="28px"
                        className="object-contain"
                      />
                    </div>
                  )}

                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-semibold text-sm text-neutral-900 dark:text-neutral-100">
                        {formatSeason(season.season)}
                      </h4>
                      {season.townHallLevel && (
                        <span className="text-[10px] px-1.5 py-0.2 font-medium rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300">
                          TH{season.townHallLevel}
                        </span>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                      <span>{season.clan?.name || 'Clan'}</span>
                      {season.clan?.warLeague?.name && (
                        <>
                          <span className="text-neutral-300 dark:text-neutral-700">•</span>
                          <span>{season.clan.warLeague.name}</span>
                        </>
                      )}
                      {season.clan?.wars && (
                        <>
                          <span className="text-neutral-300 dark:text-neutral-700">•</span>
                          <span className="font-mono text-[11px]">
                            {season.clan.wars.won}W-{season.clan.wars.lost}L{season.clan.wars.tied > 0 ? `-${season.clan.wars.tied}T` : ''}
                          </span>
                        </>
                      )}
                    </div>
                  </div>
                </div>

                {/* Season Performance & Chevron */}
                <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                  <div className="text-right">
                    <div className="text-xs font-semibold text-neutral-900 dark:text-neutral-100 font-mono">
                      <span>{seasonStars}</span>
                      <span className="text-neutral-400 text-[11px]">/{maxStars} ★</span>
                    </div>
                    <div className="text-[11px] text-neutral-500 dark:text-neutral-400">
                      {attacks.length} {attacks.length === 1 ? 'round' : 'rounds'} ({seasonAvgDestruction}%)
                    </div>
                  </div>

                  <div className="w-5 h-5 flex items-center justify-center text-neutral-400">
                    <svg
                      className={`w-4 h-4 transform transition-transform ${isExpanded ? 'rotate-180' : ''}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" />
                    </svg>
                  </div>
                </div>
              </div>

              {/* Expanded Rounds Details */}
              {isExpanded && (
                <div className="p-3.5 border-t border-neutral-100 dark:border-neutral-800/60 space-y-2">
                  <div className="flex flex-wrap items-center gap-2 pb-1 text-xs">
                    {season.placement?.clan && (
                      <span className="px-2 py-0.5 rounded text-[11px] bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
                        Player Rank: #{season.placement.clan}
                      </span>
                    )}
                    {season.clan?.placement?.group && (
                      <span className="px-2 py-0.5 rounded text-[11px] bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
                        Clan Rank: #{season.clan.placement.group}
                      </span>
                    )}
                    {season.missedAttacks > 0 && (
                      <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-rose-50 text-rose-600 dark:bg-rose-950/40 dark:text-rose-400">
                        {season.missedAttacks} Missed Attack{season.missedAttacks > 1 ? 's' : ''}
                      </span>
                    )}
                  </div>

                  {attacks.length === 0 ? (
                    <p className="text-xs text-neutral-400 italic">No attacks recorded for this season.</p>
                  ) : (
                    attacks.map((attack) => {
                      const stars = attack.stars ?? 0;
                      const destruction = attack.destructionPercentage ?? 0;

                      return (
                        <div
                          key={attack.round || attack.warTag}
                          className="p-2.5 rounded-lg border border-neutral-200/50 dark:border-neutral-800/60 bg-white/50 dark:bg-neutral-900/40"
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 mb-1 text-xs">
                            <div className="flex items-center gap-2">
                              <span className="font-semibold text-[11px] px-1.5 py-0.2 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300">
                                Round {attack.round}
                              </span>
                              {attack.opponent?.name && (
                                <span className="text-neutral-600 dark:text-neutral-300">
                                  vs {attack.opponent.name}
                                </span>
                              )}
                            </div>

                            {attack.duration !== undefined && (
                              <span className="text-[11px] font-mono text-neutral-400">
                                {formatDuration(attack.duration)}
                              </span>
                            )}
                          </div>

                          <div className="flex items-center justify-between gap-2 mt-1">
                            {attack.defender ? (
                              <div className="flex items-center gap-1.5 text-xs text-neutral-600 dark:text-neutral-300">
                                <ShieldIcon />
                                <span>{attack.defender.name}</span>
                                {attack.defender.mapPosition && (
                                  <span className="text-neutral-400 font-mono text-[10px]">
                                    (#{attack.defender.mapPosition})
                                  </span>
                                )}
                                {attack.defender.townHallLevel && (
                                  <span className="text-[10px] px-1 rounded bg-neutral-100 dark:bg-neutral-800 text-neutral-500 font-semibold">
                                    TH{attack.defender.townHallLevel}
                                  </span>
                                )}
                              </div>
                            ) : (
                              <span className="text-xs text-neutral-400">Target unknown</span>
                            )}

                            <div className="flex items-center gap-1.5 shrink-0">
                              <div className="flex items-center gap-0.5">
                                {[0, 1, 2].map((starIdx) => {
                                  const earned = starIdx < stars;
                                  return (
                                    <svg
                                      key={starIdx}
                                      className={`w-3.5 h-3.5 ${
                                        earned
                                          ? 'text-amber-400 fill-amber-400'
                                          : 'text-neutral-200 dark:text-neutral-800 fill-neutral-200 dark:fill-neutral-800'
                                      }`}
                                      viewBox="0 0 24 24"
                                    >
                                      <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                                    </svg>
                                  );
                                })}
                              </div>

                              <span className="text-xs font-bold font-mono text-neutral-900 dark:text-neutral-100">
                                {destruction}%
                              </span>
                            </div>
                          </div>
                        </div>
                      );
                    })
                  )}
                </div>
              )}
            </div>
          );
        })}
      </div>
    </div>
  );
}
